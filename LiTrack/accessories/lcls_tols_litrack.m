disp('Use TOLS_LITRACK for LCLS and general use')
return
...
[zpos,dE_E,E_bar0,dFWpct0,ZFWmm0,z_bar0,E_barcuts0,fcut0,sigzG0] = litrack(fn,seed,0,0,0,beamline0);
sigE0 = 100*std(dE_E);
sigz  = std(zpos);
Ipk0  = 1e-9*1E-3*c/(sqrt(2*pi)*sigz);
dd0 = 100*(E_barcuts0/Ef-1);

% Do gun-timing jitter test:
% =========================
figure(1)
[zpos,dE_E,E_bar1,dFWpct1,ZFWmm1,z_bar1,E_barcuts1,fcut1,sigzG1] = litrack(fn,seed,-dz,0,0,beamline0);
sigE1 = 100*std(dE_E);
sigz  = std(zpos);
Ipk1  = 1e-9*1E-3*c/(sqrt(2*pi)*sigz);
dd1 = 100*(E_barcuts1/Ef-1);
[zpos,dE_E,E_bar2,dFWpct2,ZFWmm2,z_bar2,E_barcuts2,fcut2,sigzG2] = litrack(fn,seed, dz,0,0,beamline0);
sigE2 = 100*std(dE_E);
sigz  = std(zpos);
Ipk2  = 1e-9*1E-3*c/(sqrt(2*pi)*sigz);
...
% =====================
[zpos,dE_E,E_bar1,dFWpct1,ZFWmm1,z_bar1,E_barcuts1,fcut1,sigzG1] = litrack(fn,seed,0,-dq,0,beamline0);
sigE1 = 100*std(dE_E);
sigz  = std(zpos);
Ipk1  = 1e-9*(1-dq)*1E-3*c/(sqrt(2*pi)*sigz);
dd1 = 100*(E_barcuts1/Ef-1);
[zpos,dE_E,E_bar2,dFWpct2,ZFWmm2,z_bar2,E_barcuts2,fcut2,sigzG2] = litrack(fn,seed,0, dq,0,beamline0);
sigE2 = 100*std(dE_E);
sigz  = std(zpos);
Ipk2  = 1e-9*(1+dq)*1E-3*c/(sqrt(2*pi)*sigz);
dd2 = 100*(E_barcuts2/Ef-1);
...
% ============================
  ...
  [zpos,dE_E,E_bar1,dFWpct1,ZFWmm1,z_bar1,E_barcuts1,fcut1,sigzG1] = litrack(fn,seed,0,0,0,beamline);
  sigE1 = 100*std(dE_E);
  sigz  = std(zpos);
  Ipk1  = 1e-9*1E-3*c/(sqrt(2*pi)*sigz);
  dd1 = 100*(E_barcuts1/Ef-1);
