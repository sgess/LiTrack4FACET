...
%     RAB 5/16/07  Modified codes 10 and 11 so that a negative number in column 5 is -resistance/m
...
% Codes 11 and 10 redefined by Bob Bosch RAB 5/16/07 to model a resistive wake when column 6 has negative value
  if (abs(cod)==11) | (abs(cod)==10)    % ACCELERATION SECTION (11 and 10)
    ...
    wakeon = beamline(j,5);     % wakeON=1,2, wakeOFF=0, resistive wake < 0
    Lacc   = beamline(j,6);     % length of acc section (scales wake) [m]
    phir   = phi*pi/180;        % RF phase in radians
    if wakeon>0             % if wakefile is used  RAB
      ...
      dE_wakes = interp1(zc_wake,dE_wake,z,'*linear');  % interpolate between wake calc points in bunch to evaluate dE for each e-
      dE_loss = 1E-3*mean(dE_wakes);    % wake loss [GeV]
    else                                % if wake calc switched OFF, or CSR wake used
      if wakeon
          if Lacc
            iswake = 1;                     %  wake plot  RAB
          else
            iswake = 0;                     % no wake plot RAB, since plot screws up if wake is zero
          end
      else
          iswake = 0;                       % no wake plot RAB
      end
      [dE_wake,zc_wake] = r_wake(z,Lacc,Ne1,...
                          Nbin,wakeon);  % RAB calculate dE_wake in MeV from Z-coordinates, acc length, N-particles, etc.
      dE_wakes = interp1(zc_wake,dE_wake,z,'*linear'); % RAB interpolate between wake calc points in bunch to evaluate dE for each e-
      dE_loss = 1E-3*mean(dE_wakes);    % RAB wake loss [GeV]
    end
    if abs(cod) == 10                   % special case where Eacc is final energy, rather than acc-Voltage
      if wakeon
        ...
        Eacc = (Eacc - mean(E))/cos(phir);  % don't adjust linac
      end
    end
...
%          plot_spline(zc_wakemm,dE_wake,'.')
      ...
      Hy=ylabel('\Delta{\itE}/\langle{\itE}\rangle /%');
      set(Hy,'VerticalAlignment','baseline');
      title(['\langle{\itE}\rangle='     sprintf('%6.4f',Ebarcuts) ...
             ' GeV, {\itN_e}=' sprintf('%5.3f',Ne1/1E10) '\times10^{10}'])   % changed number of decimal places to 6.4 RAB
